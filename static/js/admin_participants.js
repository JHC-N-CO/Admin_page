...
function loadColumnPreferences() {
    const PREF_VERSION_KEY = 'participantsColumnPreferencesVersion';
    const PREF_VERSION = '2';
    const savedPreferences = localStorage.getItem('participantsColumnPreferences');
    const savedVersion = localStorage.getItem(PREF_VERSION_KEY);

    if (savedPreferences) {
        const preferences = JSON.parse(savedPreferences);
        if (savedVersion !== PREF_VERSION) {
            preferences.event_id = false;
        }
        const checkboxes = document.querySelectorAll('.column-selector-dropdown input[type="checkbox"]');
...
document.addEventListener('click', function (event) {
    const trigger = event.target.closest('.role-abstract-trigger');
    if (trigger) {
        event.preventDefault();
        event.stopPropagation();
        const wrap = trigger.closest('.role-abstract-wrap');
        const pop = wrap.querySelector('.role-dl-pop');
    ...
    if (!event.target.closest('.role-abstract-wrap')) {
        closeRoleDlPops();
    }
});

function closeRoleDlPops() {
    document.querySelectorAll('.role-abstract-wrap.is-open').forEach(function (wrap) {
